  ...
  implements IRepresentationIndex {
    ...
    getSegments(_up : number, _to : number) : ISegment[] {
      ...
      for (let i = 0; i < timelineLength; i++) {
        ...
        while (
          segmentTime < to &&
          !this._isSegmentOutOfPresentationTimeline(segmentTime, duration) &&
    ...
    /**
     * Check if segment is out of presentation timeline.
     * @param {number} time
     * @param {number|undefined} duration
     * @returns {boolean}
     */
    _isSegmentOutOfPresentationTimeline(time: number, duration?: number): boolean {
